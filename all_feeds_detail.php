...
<div id="feeds-container">
    ...
    $comments = json_decode(file_get_contents("https://graph.facebook.com/".$post_id."/comments?locale=id_ID&offset=".$offset."&limit=".$limit."&access_token=".$access_token));
    ...
    for($i = 0; $i < $total_comments; $i++)
    {
        ...
        (isset($comments->data[$i]->likes)) ? $like_this = $comments->data[$i]->likes : $like_this = 0;
        ...
        //likes comments
        if($comments->data[$i]->user_likes == 1)	//user already likes status, print "Tidak Suka"
            $print_likes = "<li><a class=\"unlike\" id=\"like-this".$i."\" onclick=\"unlikeAllFeedsComment('".$id."', 'like-this".$i."', 'likes-span".$i."');\">Tidak Suka</a></li>";
        else	//vice versa, print "Suka"
            $print_likes = "<li><a class=\"like\" id=\"like-this".$i."\" onclick=\"likeAllFeedsComment('".$id."', 'like-this".$i."', 'likes-span".$i."');\">Suka</a></li>";
    ...
    <div id="paging_box">
        <a id="first" class="link" onclick="changeViewAllFeedsComments('0', 'content', '<?php echo $post_id; ?>');"></a>
        <div id="paging_button">
            <ul>
                <li><a id="prev" class="link" onclick="changeViewAllFeedsComments('<?php echo ($offset-$limit); ?>', 'content', '<?php echo $post_id; ?>');"></a></li>
                <li><a id="next" class="link" onclick="changeViewAllFeedsComments('<?php echo ($offset+$limit); ?>', 'content', '<?php echo $post_id; ?>');"></a></li>
            </ul>
        </div>
    </div>
